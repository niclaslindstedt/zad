...
`man/` contains the reference-style command documentation shipped with zad — one markdown file per command. These pages are the authoritative command-level reference and rot whenever a flag, subcommand, or default changes without a matching edit.
...
|---|---|
| CLI parser (new or renamed subcommand) | Top-level `man/zad.md`; create a new `man/<cmd>.md` if missing |
| CLI parser (new flag on existing command) | The corresponding command section |
| Global flags | `man/zad.md` Global Flags section |
...
- H1 title matches `zad man <cmd>` output.
...
1. Build and run `zad man <cmd>` for every edited page.
