...
fn seed_global(home: &std::path::Path) {
    ...
    std::fs::write(&p, "scopes = [\"search\", \"playlists.read\"]\n").unwrap();
}
...
fn create_global_writes_flat_config_and_keychain() {
    ...
    bin()
        ...
        .env("SPOTIFY_CLIENT_ID", "fake-client-id")
        .env("SPOTIFY_REFRESH", "fake-refresh-token")
...
fn enable_fails_without_any_credentials() {
    ...
    bin()
        ...
        .assert()
        .failure()
        .stderr(contains("no Spotify credentials found"));
}
...
fn show_reports_effective_source_and_keychain() {
    ...
    bin()
        .env("ZAD_HOME_OVERRIDE", home.path())
        .current_dir(project.path())
        .args(["service", "show", "spotify"])
        .assert()
        .success()
        .stdout(contains("effective : global"))
        .stdout(contains("zad-test"))
        .stdout(contains("playlists.read"))
        .stdout(contains("spotify-client-id:global"))
        .stdout(contains("spotify-refresh:global"))
        .stdout(predicates::str::contains("fake-client-id").not())
...
fn json_output_for_create() {
    ...
    bin()
        ...
        .assert()
        .success()
        .stdout(contains("\"command\": \"service.create.spotify\""))
...
fn json_output_for_show() {
    ...
    bin()
        .env("ZAD_HOME_OVERRIDE", home.path())
        .current_dir(project.path())
        .args(["service", "show", "spotify", "--json"])
        .assert()
        .success()
        .stdout(contains("\"command\": \"service.show.spotify\""))
        .stdout(contains("\"effective\": \"global\""))
        .stdout(contains("\"default_playlist\": \"zad-test\""));
}
