...
use zad::config::{self, GcalServiceCfg};
use zad::error::{Result, ZadError};
use zad::secrets::{self, Scope};
use zad::service::default_dry_run_sink;
use zad::service::gcal::client::{CalendarEntry, Event, EventsListParams, GcalHttp};
use zad::service::gcal::permissions::{self as perms, EffectivePermissions, GcalFunction};
use zad::service::gcal::transport::{DryRunGcalTransport, GcalTransport};
...
async fn run_events_create(args: EventsCreateArgs) -> Result<()> {
    ...
    let start_et = match &args.start {
        Some(s) => Some(zad::service::gcal::time::parse_event_time(s)?),
        None => None,
    };
    ...
    let (days_future, mins_future) = match &start_et {
        Some(t) => (
            zad::service::gcal::time::days_from_now(t),
            zad::service::gcal::time::minutes_from_now(t),
        ),
        None => (None, None),
    };
...
fn check_hypothetical(
    ...
    let (days_future, mins_future) = match args.start.as_deref() {
        Some(s) => {
            let t = zad::service::gcal::time::parse_event_time(s)?;
            (
                zad::service::gcal::time::days_from_now(&t),
                zad::service::gcal::time::minutes_from_now(&t),
            )
