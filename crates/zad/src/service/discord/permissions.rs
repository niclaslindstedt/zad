...
/// any `ZAD_PERMISSIONS_PATH` / `ZAD_PERMISSIONS_ROOT` override and
/// the `ZAD_HOME_OVERRIDE` test hook. This is the convenience entry
/// point used by `zad-cli`; library callers that want deterministic
/// behavior independent of the surrounding process should use
/// [`load_from`] instead.
...
/// Load `EffectivePermissions` from explicit paths. Reads **no** env
/// vars; treats a `None` argument as "no file at this scope". A
/// path that exists but doesn't parse propagates the error; a path
/// that doesn't exist contributes no restrictions (mirrors the CLI's
/// "missing file = empty layer" semantics).
///
/// Use this from library callers that want deterministic permission
/// loading — multi-tenant servers, integration tests, embed-zad-in-
/// another-app scenarios. The CLI uses [`load_effective`] instead.
...
impl crate::permissions::service::PermissionsService for PermissionsService {
    ...
    fn apply_mutation(raw: &mut Self::Raw, m: &Mutation) -> Result<()> {
        ...
        // Content + time mutations route first. If neither handles it,
        // fall through to pattern edits.
        ...
        match m {
            ...
            other => Err(mutation::unsupported("discord", other)),
        }
    }
}

fn function_block_mut<'a>(
    raw: &'a mut DiscordPermissionsRaw,
    function: &str,
    ...
    Ok(match function {
        ...
        "manage" => &mut raw.manage,
        other => {
            return Err(ZadError::Invalid(format!(
                "discord permissions: unknown function `{other}`; expected one of \
                 send, read, channels, join, leave, discover, manage"
            )));
        }
    })
...
fn pattern_list_mut<'a>(
    ...
    let Some(name) = function else {
        return Err(ZadError::Invalid(format!(
            "discord permissions: pattern mutations require --function (top-level {target} lists are not a Discord schema field)"
        )));
    };
    let block = function_block_mut(raw, name)?;
    Ok(match target {
        ...
        "guild" => &mut block.guilds,
        other => {
            return Err(ZadError::Invalid(format!(
                "discord permissions: unknown target `{other}`; expected one of channel, user, guild"
            )));
        }
    })
}
