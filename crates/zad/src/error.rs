...
pub enum ZadError {
    ...
    /// HTTP 429 — or a provider-equivalent such as Google's 403
    /// `quotaExceeded` / `rateLimitExceeded` body — from a service
    /// API. Carries enough structure that `--json` output can surface
    /// the wait deadline as machine-readable fields, and the human
    /// message always points at the `--wait` flag so callers know how
    /// to recover by re-issuing the same command.
    ///
    ...
    #[error(
        "{service} rate-limited this call; wait {retry_after_seconds}s (until {retry_after_utc}). Re-run the same command with --wait to block until ready and retry automatically."
    )]
    RateLimited {
        service: &'static str,
