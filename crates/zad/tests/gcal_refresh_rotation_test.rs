//! Same shape as `ymusic_refresh_rotation_test.rs`, applied to
//! `GcalHttp`. Google's confidential-client flow does not rotate
...
use zad::service::gcal::client::GcalHttp;
...
/// The fan-out invariant for parallel gcal callers: two `GcalHttp`
/// instances sharing a cache directory (simulating two processes with
/// the same service dir). The first one performs the token refresh;
/// the second one must pick up the cached token without hitting the
/// token endpoint.
///
/// The mock server only handles ONE request. If the second client
/// also calls the token endpoint, the test would deadlock waiting for
/// a second server that never arrives.
#[tokio::test]
async fn second_client_uses_file_cache_without_refreshing() {
    ...
    let client_a = http_with("RT-INITIAL", None, &url, tmp.path().to_path_buf());
