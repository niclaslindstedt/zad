...
//! [`crate::permissions::signing::require_keychain_key`] and fails
...
use crate::permissions::trust::{TrustStore, trust_store_path};
...
fn run_init(args: InitArgs) -> Result<()> {
    ...
    if args.force {
        ...
        if path.exists() {
            std::fs::remove_file(&path).map_err(|e| crate::error::ZadError::Io {
                path: path.clone(),
                source: e,
            })?;
        }
